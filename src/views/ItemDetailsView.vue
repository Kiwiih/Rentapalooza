...
// This is a fake array that will eventually hold the items array.
...
// hard coded item while waiting for actual data
const item = ref({
    id: "1",
    title: "Namn på item",
    description: "Beskrivning av ett item. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Vestibulum bibendum tempus eros non vulputate. Nunc non turpis nulla. Suspendisse potenti. In ut varius arcu, ac lacinia odio. In dictum facilisis urna et varius. Sed eget sagittis justo. Mauris congue augue congue, ultricies leo vitae, egestas nisl. Nullam sollicitudin sed elit sed laoreet. Nam vel neque sit amet ante dictum suscipit. Nam lacinia aliquet elit, sit amet tristique velit scelerisque et.",
...
<template>
    <div class="itemDetail">
        ...
        <div class="category">
            <span v-for="(category, index) in item.category" :key="index">{{ category }}</span>
        </div>

        <div class="imageDiv">
            <img v-for="(image, index) in item.images" :key="index" :src="image" :alt="'Bild på item ' + (index + 1)" class="itemImage">
        </div>

        <p class="desc">{{ item.description }}</p>
        <p class="price">{{  item.price }} kr</p>
        <p :class="item.isAvailable ? 'available' : 'unavailable'">{{ item.isAvailable ? "Available for rent" : "Currently unavailable" }}</p>
        <button v-if="item.isAvailable">Rent now!</button>
    </div>
</template>
